...
import { tickClock } from '../../reducer/actions/move';
import './CheckersClock.scss';
...
const PlayerTimer = ({ color, timeMs, isActive }) => {
  ...
  const classNames = [
    'checkers-timer-card',
    `checkers-timer-card--${color}`,
    isActive ? 'checkers-timer-card--active' : '',
    isLow ? 'checkers-timer-card--low' : '',
    isCritical ? 'checkers-timer-card--critical' : '',
  ].filter(Boolean).join(' ');

  return (
    <div className={classNames}>
      <div className="checkers-timer-card__header">
        <span className="checkers-timer-card__label">{label}</span>
        {isActive && <span className="checkers-timer-card__live-dot" />}
      </div>

      <div className="checkers-timer-card__display">
        <span className="checkers-timer-card__digit">{minutes}</span>
        <span className="checkers-timer-card__separator">:</span>
        <span className="checkers-timer-card__digit">{seconds}</span>
      </div>

      <div className="checkers-timer-card__bar-track">
        <div
          className="checkers-timer-card__bar-fill"
          style={{ width: `${percent}%` }}
        />
      </div>
    </div>
...
const CheckersClock = () => {
  ...
  return (
    <div className="checkers-clock">
      <PlayerTimer
        color="white"
        timeMs={whiteTime}
        isActive={turn === 'white' && isRunning}
      />
      <div className="checkers-clock__vs">VS</div>
      <PlayerTimer
        color="black"
        timeMs={blackTime}
